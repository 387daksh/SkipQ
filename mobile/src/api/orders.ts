...
export const getLastOrder = async () => {
    // Ideally user-specific endpoint. Assuming /api/orders returns user's orders sorted DESC.
    const response = await client.get('/api/orders?limit=1');
    // If API returns a list, return the first one.
    if (Array.isArray(response.data)) {
        return response.data[0];
    }
    return response.data; // If it returns object (unlikely but safe)
};

// Fetches recent unique items from the backend
export const getRecentItems = async () => {
    try {
        const response = await client.get('/api/orders/recent');
        return Array.isArray(response.data) ? response.data : [];
    } catch (e) {
        console.warn('[API] Failed to fetch recent items:', e);
        return [];
    }
};
