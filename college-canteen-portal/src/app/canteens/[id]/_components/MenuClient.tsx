...
import { ArrowLeft, Search, Plus, Minus, ShoppingBag, Loader, Loader2 } from 'lucide-react'
import { MobileCartPill } from './MobileCartPill'

type Item = { id: string; name: string; priceCents: number; imageUrl?: string | null; description?: string | null; sectionId?: string | null; sortOrder?: number | null; available?: boolean | null }
type Section = { id: string; name: string }

interface MenuClientProps {
  initialItems: Item[]
  initialSections: Section[]
  initialCanteen: { id: string; name: string; imageUrl?: string | null } | null
  canteenId: string
}

export default function MenuClient({ initialItems, initialSections, initialCanteen, canteenId }: MenuClientProps) {
  const searchParams = useSearchParams()
  const [items] = useState<Item[]>(initialItems)
  const [sections] = useState<Section[]>(initialSections)
  const [canteen] = useState(initialCanteen)
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!payload || payload.canteenId !== canteenId) return
  ...
  }, [cart, fulfillmentType, canteenId, cookingInstruction])
  ...
  const order = async () => {
    ...
    const payload = { canteenId: canteenId, items: orderItems, fulfillmentType, cookingInstructions: cookingInstruction }
    ...
    if (res.status === 401) {
      persistCartForLogin()
      const nextPath = `/canteens/${canteenId}?resume=1`
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="flex flex-col gap-4">
          <NextLink href="/canteens" className="inline-flex items-center gap-2 text-sm font-black uppercase tracking-wider hover:underline w-fit">
             ...
             <div className="bg-white border-4 border-black rounded-2xl shadow-[8px_8px_0px_0px_#000] p-6 sticky top-24">
                 ...
                 {Object.keys(cart).length === 0 ? (
                     ...
                     <div className="space-y-4">
                         ...
                         >
                             {isCheckingOut ? <Loader size={16} /> : 'Review Order'}
                         </button>
                     </div>
                 )}
             </div>
